...
2. **Check all models** lists every registered module (detection, OCR, inpainting, translation) with status: Downloaded / Missing / Hash mismatch / **No file list** (optional pip-install modules, e.g. Nemotron). The Details column shows install hints for optional modules (e.g. `pip install nemotron-ocr`).
...
3. **Series glossary** — When a **series context path** is set (project or translator param), the app loads `data/translation_context/<series_id>/glossary.txt`. Format: one line per entry, `source -> target` or `source = target`. Use `target1 | target2` to allow multiple acceptable translations (e.g. `楚州 -> Chu Province | Chuzhou`); the checker accepts any variant. For 楚州, "Chuzhou" is always accepted even if the glossary only lists "Chu Province". Lines starting with `#` are ignored. Merged with translator and project glossaries; first occurrence of each source term wins.
