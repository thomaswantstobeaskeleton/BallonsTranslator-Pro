...
2. **First run:** Installs base deps. When `config.json` is missing (e.g. fresh install), a **model package selector** appears: choose which packages to download (Core only, or Core + Advanced OCR / Advanced inpainting, etc.). Only selected packages are downloaded at startup to save disk and bandwidth (e.g. PaddleOCR-VL for manga is ~1.7 GB and is in **Advanced OCR**). You can download more later via **Tools → Manage models**. If `config.json` already existed, all models are downloaded as before (legacy). Config loads from `config/config.example.json` when missing; transient download errors are retried; manual download paths are shown in the log.
