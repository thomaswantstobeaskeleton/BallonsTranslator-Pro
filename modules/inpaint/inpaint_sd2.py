"""
Stable Diffusion 2 inpainting – 768×768 quality option via Hugging Face Diffusers.
stabilityai/stable-diffusion-2-inpainting. Between SD 1.5 (512) and SDXL (1024).
...
try:
    ...
    logging.getLogger("BallonTranslator").debug(
        "SD2 inpainting not available. Install: pip install diffusers transformers accelerate"
    )


if _DIFFUSERS_AVAILABLE:

    @register_inpainter("diffusers_sd2_inpaint")
    class DiffusersSD2Inpainter(InpainterBase):
        """
        Stable Diffusion 2 inpainting (768). Quality option between SD 1.5 and SDXL.
        """
        ...
        params = {
            "model_name": {
                ...
                "description": "SD2 inpainting model (768).",
            },
            "device": DEVICE_SELECTOR(),
            "inpaint_size": {
                "type": "line_editor",
                "value": 768,
                "description": "Max side (768 recommended; 512 faster).",
            },
            "prompt": {
                "type": "line_editor",
            ...
            "num_inference_steps": {
                "type": "line_editor",
                "value": 30,
                "description": "Denoising steps (25–40).",
            },
            "strength": {
                "type": "line_editor",
                "value": 0.99,
            ...
            "description": "SD2 inpainting 768. Install: pip install diffusers accelerate",
        }
        ...
        def _load_model(self):
            model_name = (self.params.get("model_name") or {}).get("value", "stabilityai/stable-diffusion-2-inpainting") or "stabilityai/stable-diffusion-2-inpainting"
        ...
        def _inpaint(self, img: np.ndarray, mask: np.ndarray, textblock_list: List[TextBlock] = None) -> np.ndarray:
            ...
            try:
                ...
                if isinstance(vs, dict):
                    try:
                        size = max(512, min(768, int(vs.get("value", 768))))
                ...
                if isinstance(st, dict):
                    try:
                        steps = max(20, min(50, int(st.get("value", 30))))
                    except (TypeError, ValueError):
                        pass
                strength = 0.99
                ...
                self.logger.error(f"SD2 inpainting failed: {e}")
                return img.copy()
