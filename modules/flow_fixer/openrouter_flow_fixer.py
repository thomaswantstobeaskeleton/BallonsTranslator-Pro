"""
OpenRouter flow fixer: uses a second (usually smaller/cheaper) OpenRouter model
to improve subtitle flow after translation. Use with LLM API (OpenRouter) for
translation + a small free model here for flow only.
"""
...
@register_flow_fixer(name="openrouter")
class OpenRouterFlowFixer(BaseFlowFixer):
    """Flow fixer that calls OpenRouter with a small model (e.g. Gemma-3n 2B, Qwen3-4B) for flow only."""
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.model = (model or "").strip() or "google/gemma-3n-e2b-it:free"
    ...
    def _get_client(self):
        if self._client is not None:
            return self._client
        if not self.api_key:
            LOGGER.warning("Flow fixer (OpenRouter): no API key set; flow fixer disabled.")
            return None
        try:
            import openai
            from utils.proxy_utils import create_httpx_client
            http_client = create_httpx_client(proxy=self.proxy) if self.proxy else None
            ...
            LOGGER.warning("Flow fixer (OpenRouter): could not create client: %s", e)
            return None
    ...
    def improve_flow(
            ...
            try:
                ...
                if sum_resp and sum_resp.choices:
                    ...
                    if summary_lines:
                        ...
                        LOGGER.debug("Flow fixer (OpenRouter): summarized older context to %d lines", len(prev_lines))
            except Exception as e:
                LOGGER.debug("Flow fixer (OpenRouter): summarization failed, using full context: %s", e)
        ...
        if not response or not response.choices:
            LOGGER.debug("Flow fixer (OpenRouter): empty response or no choices")
            return previous_entries, new_translations
        content = (response.choices[0].message.content or "").strip()
        if not content:
            LOGGER.debug("Flow fixer (OpenRouter): empty message content")
            return previous_entries, new_translations
