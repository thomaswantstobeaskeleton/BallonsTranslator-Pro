...
from .context_builder import (
    ...
    get_flow_fixer_response_format,
...
class LocalServerFlowFixer(BaseFlowFixer):
    ...
    def improve_flow(
        ...
        response_format = get_flow_fixer_response_format(n_prev, n_new)
        try:
            ...
            err_str = str(e).lower()
            if "response_format" in err_str or "json_schema" in err_str or "structured" in err_str:
                LOGGER.debug("Flow fixer (local): server rejected structured output, retrying without schema: %s", e)
                try:
                    response = client.chat.completions.create(
                        model=self.model,
                        messages=[
                    ...
                    LOGGER.debug("Flow fixer (local): request failed: %s", e2)
                    return previous_entries, new_translations
            else:
        ...
        if data is None:
            # One retry with minimal JSON-only prompt when model returned reasoning
            if content.strip().lower().startswith("</think>") or (
                ...
                try:
                    ...
                    LOGGER.debug("Flow fixer (local): retry failed: %s", e)
            if data is None:
                if parse_err and parse_err != "empty response":
                    if "reasoning" in (parse_err or "").lower():
                        LOGGER.info("Flow fixer (local): %s", parse_err)
                    else:
        ...
        if stats.get("prev_reverted") or stats.get("new_reverted"):
            LOGGER.debug(
                "Flow fixer (local): safety gate reverted %d previous, %d new line(s)",
        ...
        if n_prev_changed or n_new_changed:
            LOGGER.debug("Flow fixer (local): revised %d previous, %d new line(s)", n_prev_changed, n_new_changed)
