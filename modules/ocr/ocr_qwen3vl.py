"""
Qwen3-VL OCR – vision-language OCR with model size selector (2B / 4B / 8B / 32B).
Uses Hugging Face Qwen3-VL Instruct models for text extraction from cropped regions.
Requires: pip install transformers torch pillow accelerate
Detection: HF object-detection pipeline uses DETR/RT-DETR (bounding boxes); Qwen3-VL is a VLM
and does not output boxes natively. Use existing detectors (e.g. hf_object_det) for detection,
then this module for OCR.
"""
...
try:
    ...
    logging.getLogger("BallonTranslator").debug(
        f"Qwen3-VL OCR not available: {e}. Install: pip install transformers torch pillow accelerate"
    )
...
# Official Qwen3-VL Instruct model IDs (dense). MoE variants (e.g. 30B-A3B) can be added later.
QWEN3VL_MODEL_OPTIONS = [
...
if _QWEN3VL_AVAILABLE:

    @register_OCR("qwen3vl_ocr")
    class Qwen3VLOCR(OCRBase):
        """
        Qwen3-VL: vision-language OCR with selectable model size (2B / 4B / 8B / 32B).
        Use with any detector (e.g. hf_object_det). Smaller models = less VRAM, faster; larger = better quality.
        """
        params = {
            "model_name": {
                "type": "selector",
                "options": QWEN3VL_MODEL_OPTIONS,
                "value": "Qwen/Qwen3-VL-2B-Instruct",
                "description": "Model size: 2B (fast, ~6GB), 4B, 8B, 32B (best quality, high VRAM).",
            },
            ...
            "description": "Qwen3-VL OCR – select 2B/4B/8B/32B. Use with hf_object_det or other detector.",
        }
        ...
        def _run_one(self, pil_img: "Image.Image") -> str:
            ...
            try:
                ...
                self.logger.warning(f"Qwen3-VL OCR failed: {e}")
                return ""
            finally:
