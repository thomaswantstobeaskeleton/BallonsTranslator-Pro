...
_extract_classes_bboxes = None
_transform_bbox_to_original = None
...
try:
    ...
    try:
        ...
        _extract_classes_bboxes = getattr(mod, "extract_classes_bboxes", None)
        _transform_bbox_to_original = getattr(mod, "transform_bbox_to_original", None)
    except Exception:
        pass
except ImportError:
    pass
...
def _cv2_to_pil(img: np.ndarray) -> "PILImage.Image":
    if img.ndim == 2:
...
if _NEMOTRON_AVAILABLE and _extract_classes_bboxes and _transform_bbox_to_original:

    @register_OCR("nemotron_ocr")
    class NemotronOCR(OCRBase):
        """
        ...
        def _run_page(self, img: np.ndarray) -> Tuple[List[str], List[Tuple[float, float, float, float]], List[str]]:
            ...
            classes, bboxes, texts = _extract_classes_bboxes(generated)
            orig_bboxes = []
            for bbox in bboxes:
                try:
                    ob = _transform_bbox_to_original(bbox, w, h)
    ...
    logging.getLogger("BallonTranslator").debug(
        "Nemotron OCR not available. Install: pip install transformers accelerate torch albumentations timm; postprocessing from nvidia/NVIDIA-Nemotron-Parse-v1.1."
    )
