...
from qtpy.QtGui import QImage, QPixmap, QAction, QKeySequence, QPainter, QColor, QPen, QIcon
...
class VideoSubtitleEditorWindow(QMainWindow):
    ...
    def __init__(self, parent=None, video_path: str = "", subtitle_path: str = ""):
        ...
        open_video_act = QAction(QIcon(osp.join("icons", "openbtn.svg")), self.tr("Open video..."), self)
        open_video_act.triggered.connect(self._open_video)
        toolbar.addAction(open_video_act)
        load_srt_act = QAction(QIcon(osp.join("icons", "subtitle_load.svg")), self.tr("Load subtitles..."), self)
        load_srt_act.triggered.connect(self._load_subtitles)
        toolbar.addAction(load_srt_act)
        toolbar.addSeparator()
        save_srt_act = QAction(QIcon(osp.join("icons", "save_activate.svg")), self.tr("Save SRT..."), self)
        save_srt_act.triggered.connect(self._save_srt)
        toolbar.addAction(save_srt_act)
        export_ass_act = QAction(QIcon(osp.join("icons", "subtitle_export.svg")), self.tr("Export ASS..."), self)
        export_ass_act.triggered.connect(self._export_ass)
        toolbar.addAction(export_ass_act)
        export_vtt_act = QAction(QIcon(osp.join("icons", "subtitle_export.svg")), self.tr("Export WebVTT..."), self)
        export_vtt_act.triggered.connect(self._export_vtt)
        toolbar.addAction(export_vtt_act)
        toolbar.addSeparator()
        render_act = QAction(QIcon(osp.join("icons", "video_render.svg")), self.tr("Render video (burn subtitles)..."), self)
        render_act.triggered.connect(self._render_video)
        toolbar.addAction(render_act)
        toolbar.addSeparator()
        self.undo_act = QAction(QIcon(osp.join("icons", "arrow-left.svg")), self.tr("Undo"), self)
        ...
        self.redo_act = QAction(QIcon(osp.join("icons", "arrow-right.svg")), self.tr("Redo"), self)
        ...
        ctrl = QHBoxLayout()
        self.play_btn = QPushButton(QIcon(osp.join("icons", "play-solid.svg")), self.tr("Play"))
        ...
        btn_row = QHBoxLayout()
        add_btn = QPushButton(QIcon(osp.join("icons", "segment-add.svg")), self.tr("Add segment"))
        add_btn.clicked.connect(self._add_segment)
        delete_btn = QPushButton(QIcon(osp.join("icons", "segment-delete.svg")), self.tr("Delete"))
        delete_btn.clicked.connect(self._delete_segment)
        split_btn = QPushButton(QIcon(osp.join("icons", "segment-split.svg")), self.tr("Split at playhead"))
        ...
        self.set_in_btn = QPushButton(QIcon(osp.join("icons", "inout-set.svg")), self.tr("Set at playhead"))
        ...
        self.set_out_btn = QPushButton(QIcon(osp.join("icons", "inout-set.svg")), self.tr("Set at playhead"))
        self.set_out_btn.clicked.connect(self._set_out_at_playhead)
        inout_row.addWidget(self.set_out_btn)
        clear_inout_btn = QPushButton(QIcon(osp.join("icons", "inout-clear.svg")), self.tr("Clear In/Out"))
