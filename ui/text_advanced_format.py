...
class TextAdvancedFormatPanel(PanelArea):
    ...
    def __init__(self, panel_name: str, config_name: str, config_expand_name: str, on_format_changed: Callable):
        ...
        self.perspective_x_box = SmallSizeComboBox([-1, 1], 'perspective_x', self, init_value=0.)
        self.perspective_x_box.setToolTip(self.tr("3D perspective horizontal tilt"))
        self.perspective_x_box.param_changed.connect(self.on_format_changed)
        perspective_x_label = SmallParamLabel(self.tr('Perspective X'))
        self.perspective_y_box = SmallSizeComboBox([-1, 1], 'perspective_y', self, init_value=0.)
        self.perspective_y_box.setToolTip(self.tr("3D perspective vertical tilt"))
        self.perspective_y_box.param_changed.connect(self.on_format_changed)
        perspective_y_label = SmallParamLabel(self.tr('Perspective Y'))
        perspective_layout = QHBoxLayout()
        perspective_layout.addWidget(perspective_x_label)
        perspective_layout.addWidget(self.perspective_x_box)
        perspective_layout.addWidget(perspective_y_label)
        perspective_layout.addWidget(self.perspective_y_box)
        perspective_layout.addStretch(1)

        self.extrusion_depth_box = SmallSizeComboBox([0, 1], 'extrusion_depth', self, init_value=0.)
        self.extrusion_depth_box.setToolTip(self.tr("3D extrusion depth"))
        self.extrusion_depth_box.param_changed.connect(self.on_format_changed)
        extrusion_depth_label = SmallParamLabel(self.tr('3D Depth'))
        extrusion_layout = QHBoxLayout()
        extrusion_layout.addWidget(extrusion_depth_label)
        extrusion_layout.addWidget(self.extrusion_depth_box)
        extrusion_layout.addStretch(1)

        vlayout.addLayout(blend_layout)
        vlayout.addLayout(overlay_opacity_layout)
        vlayout.addLayout(skew_layout)
        vlayout.addLayout(perspective_layout)
        vlayout.addLayout(extrusion_layout)
    ...
    def set_active_format(self, font_format: FontFormat):
        ...
        self.perspective_x_box.setValue(getattr(font_format, 'perspective_x', 0.0))
        self.perspective_y_box.setValue(getattr(font_format, 'perspective_y', 0.0))
        self.extrusion_depth_box.setValue(getattr(font_format, 'extrusion_depth', 0.0))
