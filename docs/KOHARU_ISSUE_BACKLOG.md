...
_Last refreshed: 2026-05-07 via GitHub REST API against `mayocream/koharu` issues/PRs, 249 non-PR all-state issues scanned (plus prior historical PR references retained). This backlog is maintained as an implementation source, not a one-time audit._

## Newly implemented / advanced in this pass (2026-05-07 mask-aware overflow follow-up)

GitHub issue refresh: this pass refreshed `/tmp/koharu_issues_refresh.json` with the GitHub REST API across 249 non-PR all-state Koharu issues. Topic harvesting found 132 rendering/text/mask/overflow matches, 45 workflow/batch/API matches, 30 export/PSD/layer matches, 66 settings/provider/model matches, and 34 UX/editor/keybind matches. The implementation below prioritizes the user's requested text boxes, masking, and overflow work while preserving workflow/API improvements.

| Issue | Title | Category | Relevant labels | Maps to BT-Pro? | Implemented here? | Priority | Implementation notes | Deferred reason |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [#646](https://github.com/mayocream/koharu/issues/646) | Eraser tool don't work | Bugs that imply missing features; UI/UX/editor workflow; Text fitting / layout / overflow | none | yes | **advanced** | P0 | Added text-mask-visible bounds diagnostics, edge-erased mask warnings, mask-aware overflow detection, and a user-visible **Fit box to visible mask area** repair action so erased/clipped textbox regions are no longer silent. | Full eraser stroke UX rewrite and pixel-level text-mask editor improvements are deferred. |
| [#636](https://github.com/mayocream/koharu/issues/636) | Text box forcibly snaps to center of speech bubble and cannot be moved | UI/UX/editor workflow; Text fitting / layout / overflow | none | yes | **advanced** | P0 | Added centered growth for mask-safe textbox resizing and layout-review actions that preserve textbox center while expanding only enough for visible lettering/mask bounds. | More explicit lock/unlock affordances for bubble-centering are deferred. |
| [#649](https://github.com/mayocream/koharu/issues/649) / [#648](https://github.com/mayocream/koharu/issues/648) | Global font-size override / modify entire project at once | Text rendering / typography / fonts; Batch/project workflow | ui, renderer, feature request | yes | **advanced** | P0 | Typography QA and project rendering fixes can now report/apply mask-safe textbox growth in addition to font shrinking, line balancing, padding, and style updates. | Before/after thumbnails for project-wide QA fixes remain deferred. |
| [#612](https://github.com/mayocream/koharu/pull/612) | Granular automation workflow | Automation/API/headless/MCP; Batch/project workflow | area: ui | yes | **advanced** | P1 | Local API rendering issue rows now expose mask diagnostics and visible-area ratios, allowing headless agents to prioritize textboxes where masks and overflow interact. | Event streaming/progress subscription remains deferred. |

## Newly implemented / advanced in this pass (2026-05-07 workflow presets + vertical lettering hints)

GitHub issue refresh: `gh` was unavailable, so this pass refreshed `/tmp/koharu_issues.json` with the GitHub REST API across 249 non-PR all-state Koharu issues, then prioritized current open feature/workflow/renderer requests including #594, #597, #592, #612, #601, #649, #648, #610, #602, #595, and #593.

| Issue | Title | Category | Relevant labels | Maps to BT-Pro? | Implemented here? | Priority | Implementation notes | Deferred reason |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [#594](https://github.com/mayocream/koharu/issues/594) / [#597](https://github.com/mayocream/koharu/issues/597) | Advanced text formatting / manual text direction toggle | Text rendering / typography / fonts; Vertical CJK / RTL / punctuation | renderer, feature request, font | yes | **advanced** | P0 | Tightened kinsoku line-breaking with Japanese small-kana/prolongation/iteration mark line-start bans; fixed tate-chu-yoko vertical run detection so compact two-character groups are index-precise instead of character-match-based; structured OCR/headless exports now include resolved writing mode, vertical columns, tate-chu-yoko groups, bracket pairs, padding/spacing/fallback/preset metadata. | True HarfBuzz/OpenType vertical alternates and Arabic shaping remain deferred to a dependency-backed renderer pass. |
| [#592](https://github.com/mayocream/koharu/issues/592) | Feature: Workflow presets | Batch/project workflow; Onboarding/setup workflow; UI/UX/editor workflow; Automation/API/headless/MCP | feature request, workflow | yes | **partial → advanced** | P1 | Added a reusable workflow-preset backend (`full`, `detect_ocr`, `translate`, `inpaint`, `lettering_review`), a Pipeline Insights combo with **Apply** and **Apply + Run** buttons, persistence through existing config save, stage-action synchronization, and automation `pipeline_presets` list/apply/run actions. | A richer user-defined workflow-preset editor and per-project preset sharing remain deferred. |
| [#612](https://github.com/mayocream/koharu/pull/612) / [#649](https://github.com/mayocream/koharu/issues/649) / [#648](https://github.com/mayocream/koharu/issues/648) | Granular workflow automation / global style cleanup | Automation/API/headless/MCP; UI/UX/editor workflow; Text fitting / layout / overflow | area: ui, renderer, feature request | yes | **advanced** | P1 | The new `pipeline_presets` route gives headless clients fewer-click staged pipeline control and a no-AI lettering QA preset that pairs with existing rendering QA/layout review endpoints. Structured OCR render hints improve cross-page agents that need ordered textboxes plus typography state. | Event-streamed API progress and model/provider setup wizard remain deferred. |
| [#601](https://github.com/mayocream/koharu/issues/601) | Drag to reorder text boxes | UI/UX/editor workflow | ui, feature request | yes | **supporting improvement** | P1 | Existing side-panel reorder/read-order work is now complemented by richer structured OCR render hints, preserving source index + resolved reading/rendering data for downstream reorder/export tools. | Canvas-side drag-to-reorder overlay remains deferred. |
